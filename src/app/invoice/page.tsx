...
export default function InvoicePage() {
  ...
  function createNewInvoice() {
    ...
    showToast(t('✅ Nouvelle facture créée', '✅ New invoice created'))
  }
  ...
  function importPunchDay(date: string) {
    ...
    const desc = `${t('Travail', 'Work')} — ${fmtDate(date)}${detail.notes ? ' — ' + detail.notes : ''}`
    ...
    showToast(`✅ ${fmtDate(date)} ${t('importé', 'imported')} (${hours.toFixed(2)}h)`)
  ...
  function addFromCatalogue(mat: { name: string; price: number; unit?: string }) {
    if (!openInvoiceId) return
    invoiceStore.addLineItem(openInvoiceId, {
      description: mat.name, quantity: 1, unitPrice: mat.price, total: mat.price,
    })
    showToast(`✅ ${mat.name} ${t('ajouté', 'added')}`)
    setShowCatalogue(false); setCatSearch('')
  }
  ...
  function handleEmail() {
    ...
    const subject = encodeURIComponent(`${t('Facture', 'Invoice')} ${openInvoice.number} — ${viewEmployee.name}`)
    const body = encodeURIComponent(
      `${t('Bonjour', 'Hello')},\n\n${t('Veuillez trouver ci-joint ma facture', 'Please find attached my invoice')} ${openInvoice.number} ${t("d'un montant de", 'for the amount of')} ${fmt(openInvoice.total)}.\n\n${viewEmployee.name}\n${company.phone || ''}`
    )
    window.open(`mailto:${openInvoice.clientEmail || ''}?subject=${subject}&body=${body}`)
    upd({ status: 'envoye' })
    showToast(t('📧 Email ouvert', '📧 Email opened'))
  }
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h1 style={{ fontSize: '20px', fontWeight: 800, margin: 0, color: pc }}>
                🧾 {t('Mes Factures', 'My Invoices')}
              </h1>
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: '2px 0 0' }}>
                {viewEmployee?.name} — {myInvoices.length} {t('facture', 'invoice')}{myInvoices.length !== 1 ? t('s', 's') : ''}
              </p>
            </div>
          ...
          {myInvoices.length === 0 ? (
            <div style={{ ...cardStyle, textAlign: 'center', padding: '40px 20px' }}>
              ...
              <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--text)', marginBottom: '6px' }}>
                {t('Aucune facture', 'No invoices')}
              </div>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '20px' }}>
                {t('Créez votre première facture — vos journées de travail s\'importent automatiquement', 'Create your first invoice — your work days import automatically')}
              </div>
              <button onClick={createNewInvoice} style={btnPrimary}>
                + {t('Créer ma première facture', 'Create my first invoice')}
              </button>
            </div>
          ) : (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>
                      {fmtDate(inv.date)} · {inv.items.length} {t('ligne', 'line')}{inv.items.length !== 1 ? 's' : ''}
                    </div>
  ...
  // ── VUE DÉTAIL FACTURE ────────────────────────────────────────────────────────
  const catFiltered = materials.filter(m => m.name.toLowerCase().includes(catSearch.toLowerCase()))
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', paddingBottom: '120px' }}>
      ...
      <div style={{ padding: '16px' }}>
        ...
        {/* Info de base */}
        <div style={cardStyle}>
          <DocumentWatermark type="FACTURE" employeeName={openInvoice.employeeName} opacity={0.07}/>
        ...
        <div style={cardStyle}>
          <div style={{ fontSize: '11px', color: pc, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '12px' }}>
            👤 {t('Facturer à', 'Bill To')}
          </div>
          <div style={{ marginBottom: '8px' }}>
            <label style={lbl}>{t('Nom', 'Name')}</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              <label style={lbl}>{t('Courriel', 'Email')}</label>
            ...
            <div>
              <label style={lbl}>{t('Téléphone', 'Phone')}</label>
        ...
        <div style={cardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
            <div style={{ fontSize: '11px', color: pc, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              🔧 {t('Articles & Heures', 'Items & Hours')}
            </div>
            <div style={{ display: 'flex', gap: '6px' }}>
              <button onClick={() => setShowPunchImport(!showPunchImport)} style={{ padding: '6px 10px', borderRadius: '8px', cursor: 'pointer', background: '#22c55e18', border: '1px solid #22c55e44', color: '#22c55e', fontSize: '11px', fontWeight: 700 }}>
                ⏱ {t('Punch', 'Punch')}
              </button>
              <button onClick={() => setShowCatalogue(!showCatalogue)} style={{ padding: '6px 10px', borderRadius: '8px', cursor: 'pointer', background: `${pc}18`, border: `1px solid ${pc}44`, color: pc, fontSize: '11px', fontWeight: 700 }}>
          ...
          {showPunchImport && (
            <div style={{ background: 'var(--surface)', borderRadius: '10px', padding: '12px', marginBottom: '12px', border: '1px solid #22c55e44' }}>
              <div style={{ fontSize: '11px', color: '#22c55e', fontWeight: 700, marginBottom: '10px', textTransform: 'uppercase' }}>
                ⏱ {t('Importer une journée de travail', 'Import a work day')}
              </div>
              {punchDays.length === 0 ? (
                <p style={{ fontSize: '13px', color: 'var(--text-muted)', textAlign: 'center', padding: '12px 0' }}>
                  {t('Aucune journée enregistrée', 'No days recorded')}
                </p>
              ) : punchDays.map(({ date, detail }) => (
                <button key={date} onClick={() => importPunchDay(date)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 12px', color: 'var(--text)', cursor: 'pointer', marginBottom: '6px', textAlign: 'left' }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: '13px' }}>{fmtDate(date)}</div>
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{detail.totalHours.toFixed(2)}h · {fmt(detail.totalRevenue)}</div>
                  </div>
          ...
          {showCatalogue && (
            <div style={{ background: 'var(--surface)', borderRadius: '10px', padding: '12px', marginBottom: '12px', border: `1px solid ${pc}44` }}>
              <input value={catSearch} onChange={e => setCatSearch(e.target.value)} placeholder={`🔍 ${t('Rechercher dans le catalogue...', 'Search catalogue...')}`} style={{ ...inputStyle, marginBottom: '8px' }}/>
              <div style={{ maxHeight: '180px', overflowY: 'auto' }}>
                {catFiltered.length === 0 ? (
                  <p style={{ fontSize: '13px', color: 'var(--text-muted)', textAlign: 'center' }}>{t('Aucun résultat', 'No results')}</p>
                ) : catFiltered.map(mat => (
                  <button key={mat.id} onClick={() => addFromCatalogue({ name: mat.name, price: mat.price ?? 0, unit: mat.unit })} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid var(--border)', padding: '8px 4px', color: 'var(--text)', cursor: 'pointer', textAlign: 'left' }}>
            ...
            <div key={item.id} style={{ background: 'var(--surface)', borderRadius: '10px', padding: '12px', marginBottom: '8px', border: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: 600 }}>
                  {t('Ligne', 'Line')} {idx + 1}{item.fromPunch ? ' ⏱' : ''}{item.date ? ` · ${fmtDate(item.date)}` : ''}
                </span>
                {openInvoice.items.length > 1 && (
                  <button onClick={() => invoiceStore.removeLineItem(openInvoice.id, item.id)} style={{ background: 'none', border: 'none', color: 'var(--danger, #ef4444)', cursor: 'pointer', fontSize: '18px' }}>✕</button>
                )}
              </div>
              <div style={{ marginBottom: '8px' }}>
                <label style={lbl}>{t('Description', 'Description')}</label>
                <input value={item.description} onChange={e => updLine(item.id, 'description', e.target.value)} placeholder={t('Description du travail ou matériau...', 'Work description or material...')} style={inputStyle}/>
              </div>
              {item.address !== undefined && (
                <div style={{ marginBottom: '8px' }}>
                  <label style={lbl}>{t('Adresse du chantier', 'Job site address')}</label>
                  <input value={item.address || ''} onChange={e => updLine(item.id, 'address', e.target.value)} placeholder="123 rue Exemple, Calgary" style={inputStyle}/>
                </div>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                <div>
                  <label style={lbl}>{t('Qté / Heures', 'Qty / Hours')}</label>
                  <input type="number" value={item.quantity} min="0" step="0.25" onChange={e => updLine(item.id, 'quantity', Number(e.target.value))} style={inputStyle}/>
                </div>
                <div>
                  <label style={lbl}>{t('Prix unit.', 'Unit price')}</label>
                  <input type="number" value={item.unitPrice} min="0" step="0.01" onChange={e => updLine(item.id, 'unitPrice', Number(e.target.value))} style={inputStyle}/>
                </div>
                <div>
                  <label style={lbl}>{t('Total', 'Total')}</label>
          ...
          <button onClick={() => invoiceStore.addLineItem(openInvoice.id)} style={{ width: '100%', padding: '12px', background: 'transparent', border: '2px dashed var(--border)', borderRadius: '10px', color: pc, cursor: 'pointer', fontSize: '14px', fontWeight: 600 }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ marginBottom: '12px' }}>
              <label style={lbl}>{t('Dépôt reçu ($)', 'Deposit received ($)')}</label>
              <input type="number" value={openInvoice.deposit} min="0" step="0.01"
                onChange={e => { upd({ deposit: Number(e.target.value) }); invoiceStore.calculateTotals(openInvoice.id) }} style={inputStyle}/>
            </div>
            <div style={{ background: 'var(--surface)', borderRadius: '10px', padding: '16px', border: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '14px', color: 'var(--text-muted)' }}>
                <span>{t('Sous-total', 'Subtotal')}</span><span>{fmt(openInvoice.subtotal)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '14px', color: 'var(--text-muted)' }}>
        ...
        <div style={cardStyle}>
          ...
          <textarea value={openInvoice.notes} onChange={e => upd({ notes: e.target.value })} rows={3} placeholder={t('Merci pour la confiance...', 'Thank you for your trust...')} style={{ ...inputStyle, resize: 'vertical', lineHeight: '1.6' }}/>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <canvas ref={sigRef} width={320} height={110}
            ...
            onTouchMove={e => {
              ...
              const sy = (sigRef.current?.height ?? 110) / r.height
              ctx.strokeStyle = pc; ctx.lineWidth = 2; ctx.lineCap = 'round'
              ctx.lineTo((touch.clientX - r.left) * sx, (touch.clientY - r.top) * sy)
              ctx.stroke(); ctx.beginPath()
              ctx.moveTo((touch.clientX - r.left) * sx, (touch.clientY - r.top) * sy)
            }}
          />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button style={btnPrimary} onClick={() => { showToast(t('✅ Facture sauvegardée!', '✅ Invoice saved!')) }}>
            💾 {t('Sauvegarder', 'Save')}
          </button>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            ...
            <button onClick={() => { if (confirm(t('Supprimer cette facture?', 'Delete this invoice?'))) { invoiceStore.deleteInvoice(openInvoice.id); setOpenInvoiceId(null) } }}
      ...
      {showPreview && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.92)', zIndex: 200, overflowY: 'auto' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', color: '#111', margin: '20px auto', maxWidth: '600px', borderRadius: '12px', padding: '32px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%) rotate(-22deg)', pointerEvents: 'none', zIndex: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', opacity: 0.15, userSelect: 'none', textAlign: 'center' }}>
              <div style={{ fontSize: '52px', fontWeight: 900, color: '#000', letterSpacing: '2px', lineHeight: 1.1, whiteSpace: 'nowrap' }}>{openInvoice.employeeName.toUpperCase()}</div>
              <div style={{ fontSize: '36px', fontWeight: 800, color: '#000', letterSpacing: '14px', textTransform: 'uppercase', whiteSpace: 'nowrap', marginTop: '4px' }}>{t('FACTURE', 'INVOICE')}</div>
            </div>
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                <div>
                  {company.logoUrl && <img src={company.logoUrl} alt="" style={{ width: '60px', height: '60px', objectFit: 'contain', marginBottom: '8px', display: 'block' }}/>}
                  ...
                  <div style={{ marginTop: '8px', padding: '8px', background: '#f8f8f8', borderRadius: '8px' }}>
                    ...
                    <div style={{ fontSize: '12px', color: '#666' }}>{t('Employé / Sous-traitant', 'Employee / Contractor')}</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '26px', fontWeight: 900, color: '#f59e0b', textTransform: 'uppercase' }}>{t('FACTURE', 'INVOICE')}</div>
                  <div style={{ fontSize: '14px', fontWeight: 700 }}>{openInvoice.number}</div>
                  <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>{t('Date', 'Date')}: {openInvoice.date}</div>
                  <div style={{ fontSize: '12px', color: '#666' }}>{t('Échéance', 'Due')}: {openInvoice.dueDate}</div>
                </div>
              </div>
              {openInvoice.clientName && (
                <div style={{ background: 'rgba(0,0,0,0.04)', borderRadius: '8px', padding: '14px', marginBottom: '20px' }}>
                  <div style={{ fontSize: '10px', color: '#888', textTransform: 'uppercase', marginBottom: '4px' }}>{t('Facturer à', 'Bill To')}</div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '24px' }}>
                <thead>
                  <tr style={{ background: 'rgba(17,17,17,0.85)', color: '#fff' }}>
                    <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: '12px' }}>{t('Description', 'Description')}</th>
                    <th style={{ padding: '10px 12px', textAlign: 'center', fontSize: '12px' }}>{t('Qté', 'Qty')}</th>
                    <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: '12px' }}>{t('Prix unit.', 'Unit price')}</th>
                    <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: '12px' }}>{t('Total', 'Total')}</th>
                  </tr>
                </thead>
                <tbody>
                  {openInvoice.items.map((item, i) => (
                    <tr key={item.id} style={{ background: i % 2 === 0 ? 'rgba(0,0,0,0.03)' : 'transparent' }}>
                      <td style={{ padding: '10px 12px', fontSize: '13px' }}>{item.description || '—'}{item.address && <div style={{ fontSize: '11px', color: '#888' }}>📍 {item.address}</div>}</td>
              ...
              <div style={{ marginLeft: 'auto', maxWidth: '240px', marginBottom: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px' }}>
                  <span style={{ color: '#666' }}>{t('Sous-total', 'Subtotal')}</span><span>{fmt(openInvoice.subtotal)}</span>
                </div>
                ...
                <div style={{ borderTop: '2px solid #111', paddingTop: '10px', marginTop: '10px', display: 'flex', justifyContent: 'space-between', fontSize: '20px', fontWeight: 800 }}>
                  <span>{t('TOTAL', 'TOTAL')}</span><span style={{ color: '#f59e0b' }}>{fmt(openInvoice.total)}</span>
                </div>
                {openInvoice.deposit > 0 && (
                  <>
              ...
              {(company.etransferEmail || company.bankName) && (
                <div style={{ marginBottom: '20px', padding: '14px', background: 'rgba(59,130,246,0.06)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '11px', color: '#3b82f6', textTransform: 'uppercase', marginBottom: '6px' }}>{t('Paiement', 'Payment')}</div>
              ...
              {openInvoice.notes && (
                <div style={{ marginBottom: '20px', padding: '14px', background: 'rgba(0,0,0,0.03)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '11px', color: '#888', textTransform: 'uppercase', marginBottom: '6px' }}>{t('Notes', 'Notes')}</div>
