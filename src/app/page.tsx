  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
      <style>{`
        ...
        @keyframes robotWave { 0%,100%{transform:rotate(0deg)} 25%{transform:rotate(-20deg)} 75%{transform:rotate(20deg)} }
        @keyframes robotBlink { 0%,90%,100%{transform:scaleY(1)} 95%{transform:scaleY(0.1)} }
        @keyframes robotTalk { 0%,100%{transform:scaleY(1)} 50%{transform:scaleY(0.5)} }
      ...
      <div style={{ position: 'relative' }}>
        <div className="bubble" style={{
          ...
          {message}
          <div style={{ position: 'absolute', bottom: '-6px', left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '6px solid transparent', borderRight: '6px solid transparent', borderTop: `6px solid ${happy ? '#7c3aed' : '#4c1d95'}` }}/>
        </div>
        <svg className="robot-body" width="80" height="90" viewBox="0 0 80 90" fill="none">
          ...
          <circle cx="40" cy="6" r="4" fill={happy ? '#22d3ee' : '#a855f7'} style={{ filter: `drop-shadow(0 0 4px ${happy ? '#22d3ee' : '#a855f7'})` }}/>
          <rect x="22" y="18" width="36" height="28" rx="8" fill={bodyColor} stroke="#a855f7" strokeWidth="1.5"/>
          <g className="robot-eye"><rect x="28" y="26" width="8" height="8" rx="2" fill={eyeColor} style={{ filter: `drop-shadow(0 0 4px ${eyeColor})` }}/></g>
          <g className="robot-eye" style={{ animationDelay: '0.2s' }}><rect x="44" y="26" width="8" height="8" rx="2" fill={eyeColor} style={{ filter: `drop-shadow(0 0 4px ${eyeColor})` }}/></g>
          <g className="robot-mouth">
            {happy ? <path d="M30 38 Q40 44 50 38" stroke="#22d3ee" strokeWidth="2" fill="none" strokeLinecap="round"/> : ok ? <line x1="30" y1="41" x2="50" y2="41" stroke="#a855f7" strokeWidth="2" strokeLinecap="round"/> : <path d="M30 43 Q40 38 50 43" stroke="#6b7280" strokeWidth="2" fill="none" strokeLinecap="round"/>}
          </g>
          <rect x="24" y="50" width="32" height="26" rx="6" fill={bodyColor} stroke="#7c3aed" strokeWidth="1.5"/>
          <text x="40" y="67" textAnchor="middle" fontSize="10" fontWeight="900" fill={happy ? '#22d3ee' : '#a855f7'} style={{ filter: `drop-shadow(0 0 3px ${happy ? '#22d3ee' : '#a855f7'})` }}>XP</text>
...
function WorkerCharacter({ isWorking }: { isWorking: boolean }) {
  return (
    <svg width="60" height="70" viewBox="0 0 60 70" fill="none">
      <style>{`
        @keyframes workerNod { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-3px)} }
        @keyframes workerWork { 0%,100%{transform:rotate(-5deg)} 50%{transform:rotate(5deg)} }
      ...
      <g className="worker-head">
        ...
        {isWorking ? <path d="M25 31 Q30 35 35 31" stroke="#92400e" strokeWidth="1.5" fill="none" strokeLinecap="round"/> : <path d="M25 32 Q30 28 35 32" stroke="#92400e" strokeWidth="1.5" fill="none" strokeLinecap="round"/>}
      </g>
  ...
  return (
    <div style={{ width: '100%', height: '10px', background: 'rgba(168,85,247,0.2)', borderRadius: '999px', overflow: 'hidden', position: 'relative' }}>
      <div style={{ height: '100%', borderRadius: '999px', width: `${pct}%`, background: `linear-gradient(90deg, ${color}, #22d3ee)`, boxShadow: `0 0 10px ${color}88`, transition: 'width 1s ease', position: 'relative' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.3) 50%, transparent 100%)', backgroundSize: '200% 100%', animation: 'xpShimmer 2s linear infinite', borderRadius: '999px' }}/>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
      <div style={{ width: '52px', height: '52px', background: `${color}22`, border: `2px solid ${color}66`, borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', boxShadow: `0 0 12px ${color}44`, position: 'relative' }}>
        {icon}
        <div style={{ position: 'absolute', inset: 0, borderRadius: '12px', background: `linear-gradient(135deg, ${color}11, transparent)` }}/>
      </div>
...
export default function HomePage() {
  ...
  const robotMessage = !goal ? '👋 Salut!' : goalPct >= 100 ? '🎉 OBJECTIF!' : goalPct >= 80 ? '🔥 Presque!' : goalPct >= 50 ? '💪 Continue!' : goalPct >= 20 ? '⚡ En route!' : '🎯 Allons-y!'
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', paddingTop: '8px' }}>
        <style>{`
          ...
          .emp-card:active{transform:scale(0.97)}.emp-card{transition:transform 0.15s ease}
        ...
        {isXP ? (
          <>
            <div style={{ textAlign: 'center', paddingTop: '8px' }}>
              ...
              <p style={{ color: '#6b7280', fontSize: '11px', letterSpacing: '3px', marginTop: '4px' }}>GAMING EDITION</p>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-around', padding: '4px 0' }}>
              {['⭐','✨','💜','✨','⭐'].map((s, i) => <span key={i} className="star-twinkle" style={{ fontSize: '14px', animationDelay: `${i * 0.3}s` }}>{s}</span>)}
            </div>
          </>
        ) : (
          <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: employees.filter(e => e.active).length > 3 ? '1fr 1fr' : '1fr', gap: '10px' }}>
          {employees.filter(e => e.active).map((emp, idx) => {
            ...
            return (
              <button key={emp.id} className="emp-card" onClick={() => handleSelectEmployee(emp.id)} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '16px', borderRadius: '16px', cursor: 'pointer', border: isXP ? '1px solid rgba(168,85,247,0.35)' : '1px solid var(--border)', background: isXP ? 'rgba(17,7,40,0.9)' : 'var(--card)', textAlign: 'left' as const, position: 'relative', overflow: 'hidden', animation: `floatUp 0.4s ease ${idx * 0.1}s both` }}>
                {!isXP && <DecoBackground/>}
                ...
                <div style={{ width: '46px', height: '46px', borderRadius: isXP ? '12px' : '50%', flexShrink: 0, background: isXP ? `linear-gradient(135deg, ${emp.color}, #a855f7)` : `radial-gradient(circle at 40% 35%, ${emp.color}99, ${emp.color})`, boxShadow: isXP ? `0 0 16px ${emp.color}66` : `0 0 18px ${emp.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 800, color: 'white', position: 'relative', zIndex: 1 }}>{emp.name[0].toUpperCase()}</div>
                <div style={{ flex: 1, position: 'relative', zIndex: 1 }}>
                  <p style={{ color: isXP ? '#e9d5ff' : 'var(--text)', fontSize: '15px', fontWeight: 700 }}>{emp.name}</p>
                  {isXP ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                      <span style={{ fontSize: '10px', background: 'rgba(168,85,247,0.3)', color: '#a855f7', padding: '2px 6px', borderRadius: '4px', fontWeight: 700 }}>Nv.{empGoal.level}</span>
        ...
        {isXP ? (
          <div style={{ textAlign: 'center' }}><p style={{ fontSize: '10px', letterSpacing: '2px', color: '#4c1d95', fontWeight: 700 }}>🎮 CHOISISSEZ VOTRE PERSONNAGE</p></div>
        ) : (
          <>
  ...
  if (screen === 'pin') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', paddingTop: '16px' }}>
        ...
        {isXP ? (
          <>
            <div style={{ width: '72px', height: '72px', borderRadius: '16px', background: `linear-gradient(135deg, ${emp?.color}, #a855f7)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: 800, color: 'white', boxShadow: `0 0 30px ${emp?.color}66, 0 0 60px rgba(168,85,247,0.3)` }}>{emp?.name[0].toUpperCase()}</div>
            <div style={{ textAlign: 'center' }}>
          ...
          <>
            <DecoOrnament opacity={0.12}/>
            <div style={{ width: '70px', height: '70px', borderRadius: '50%', background: `radial-gradient(circle at 40% 35%, ${emp?.color}99, ${emp?.color})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '26px', fontWeight: 800, color: 'white', boxShadow: `0 0 30px ${emp?.color}66` }}>{emp?.name[0].toUpperCase()}</div>
        ...
        <div style={{ display: 'flex', gap: '14px' }}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{ width: '18px', height: '18px', borderRadius: isXP ? '4px' : '50%', background: pin.length > i ? pinError ? '#ef4444' : (isXP ? '#a855f7' : 'var(--primary)') : (isXP ? 'rgba(168,85,247,0.15)' : 'var(--surface)'), border: `2px solid ${pinError ? '#ef4444' : isXP ? 'rgba(168,85,247,0.4)' : 'var(--border)'}`, transition: 'all 0.2s', boxShadow: pin.length > i ? isXP ? '0 0 12px rgba(168,85,247,0.8)' : '0 0 12px var(--primary)' : 'none' }}/>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', width: '100%', maxWidth: '260px' }}>
          {['1','2','3','4','5','6','7','8','9','','0','⌫'].map((d, i) => (
            <button key={i} onClick={() => { if (d === '⌫') setPin(p => p.slice(0,-1)); else if (d !== '') handlePinDigit(d) }} style={{ height: '60px', borderRadius: isXP ? '12px' : '10px', cursor: d ? 'pointer' : 'default', border: isXP ? '1px solid rgba(168,85,247,0.25)' : '1px solid var(--border)', background: d ? (isXP ? 'rgba(17,7,40,0.9)' : 'var(--card)') : 'transparent', color: isXP ? '#e9d5ff' : 'var(--text)', fontSize: d === '⌫' ? '18px' : '22px', fontWeight: 700, opacity: d ? 1 : 0 }}>{d}</button>
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', paddingBottom: '8px' }}>
      <style>{`
        ...
        @keyframes neonPulseRed{0%,100%{box-shadow:0 0 15px #ef4444,0 0 30px #ef444488}50%{box-shadow:0 0 25px #ef4444,0 0 50px #ef4444aa}}
      ...
      {showCelebration && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i} style={{ position: 'absolute', left: `${Math.random() * 100}%`, top: '-20px', fontSize: '20px', animation: `confettiFall ${1 + Math.random() * 2}s ease-in ${Math.random() * 0.5}s forwards` }}>
              {['🎉','⭐','💜','✨','🏆','💎'][Math.floor(Math.random() * 6)]}
            </div>
          ))}
          <div style={{ background: 'rgba(17,7,40,0.95)', border: '2px solid #a855f7', borderRadius: '20px', padding: '24px 32px', textAlign: 'center', boxShadow: '0 0 40px rgba(168,85,247,0.6)' }}>
      ...
      <div style={{ ...card, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: isXP ? 'rgba(17,7,40,0.95)' : 'var(--card)', border: isXP ? '1px solid rgba(168,85,247,0.4)' : '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', position: 'relative', zIndex: 1 }}>
          <div style={{ width: '40px', height: '40px', borderRadius: isXP ? '10px' : '50%', flexShrink: 0, background: isXP ? `linear-gradient(135deg, ${currentEmployee?.color}, #a855f7)` : `radial-gradient(circle at 40% 35%, ${currentEmployee?.color}99, ${currentEmployee?.color})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', fontWeight: 800, color: 'white', boxShadow: isXP ? `0 0 16px ${currentEmployee?.color}55, 0 0 30px rgba(168,85,247,0.3)` : `0 0 14px ${currentEmployee?.color}55` }}>{currentEmployee?.name[0].toUpperCase()}</div>
          <div>
            ...
            {isXP && goal ? (
              ...
              <p style={{ fontSize: '10px', color: 'var(--primary)', letterSpacing: '1px', fontWeight: 700 }}>{activeSession ? (isOnBreak ? '☕ EN PAUSE' : '🟢 EN SERVICE') : '⏸ HORS SERVICE'}</p>
            )}
          </div>
        </div>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'linear-gradient(135deg, #7c3aed, #a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 900, color: 'white', boxShadow: '0 0 12px rgba(168,85,247,0.5)' }}>{goal.level}</div>
                <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        <div style={{ ...card }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', position: 'relative', zIndex: 1 }}>
            <div style={{ width: '34px', height: '34px', borderRadius: isXP ? '8px' : '50%', border: `1px solid ${isXP ? 'rgba(168,85,247,0.4)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: isXP ? 'rgba(168,85,247,0.1)' : 'transparent' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {editingRate ? (
                ...
                <p onClick={() => { setTempRate(String(currentEmployee?.hourlyRate ?? 45)); setEditingRate(true) }} style={{ fontSize: '14px', fontWeight: 900, cursor: 'pointer', color: isXP ? '#a855f7' : 'var(--primary)' }}>${currentEmployee?.hourlyRate ?? 45}/h</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '10px' }}>
        <div style={{ ...card, background: isXP ? 'linear-gradient(135deg, #0a0514, #12082a)' : 'linear-gradient(135deg, #1a1200, #2a1f00, #1a1200)' }}>
          ...
          {revenue > 0 && [{ left: '15%', delay: '0s' }, { left: '50%', delay: '0.9s' }, { left: '80%', delay: '1.6s' }].map((coin, i) => (
            <span key={i} className="coin-fall" style={{ left: coin.left, top: 0, animationDelay: coin.delay }}>{isXP ? '💜' : '🪙'}</span>
          ))}
      ...
      {isXP ? (
        <div style={{ ...card, background: 'rgba(10,5,20,0.95)', border: '1px solid rgba(168,85,247,0.3)', padding: '20px 16px' }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', width: '300px', height: '300px', transform: 'translate(-50%, -50%)', pointerEvents: 'none', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: '50%', left: '50%', width: '280px', height: '280px', background: 'repeating-conic-gradient(rgba(168,85,247,0.06) 0deg 10deg, transparent 10deg 20deg)', borderRadius: '50%', transform: 'translate(-50%, -50%)', animation: 'raysRotate 20s linear infinite' }}/>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', zIndex: 1 }}>
            <div style={{ opacity: 0.9 }}><XPRobot percent={goalPct} message={robotMessage}/></div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
              <button onClick={isRunning ? handlePunchOut : handlePunchIn} className={isRunning ? 'neon-punch-out' : 'neon-punch'} style={{ width: '120px', height: '120px', borderRadius: '50%', background: isRunning ? 'radial-gradient(circle at 40% 35%, #f87171, #ef4444 60%, #b91c1c)' : 'radial-gradient(circle at 40% 35%, #c084fc, #a855f7 60%, #7c3aed)', border: 'none', cursor: 'pointer', color: 'white', fontSize: '13px', fontWeight: 900, letterSpacing: '1.5px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
            ...
            <div style={{ opacity: 0.9 }}><WorkerCharacter isWorking={isRunning}/></div>
          </div>
        ...
        <>
          ...
          {isRunning && !isOnBreak && currentEmployeeId && (
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button onClick={() => startBreak(currentEmployeeId)} style={{ borderRadius: '999px', border: '2px solid var(--warning)', color: 'var(--warning)', background: 'transparent', padding: '12px 32px', fontSize: '13px', cursor: 'pointer', fontWeight: 800, letterSpacing: '2px' }}>{t('☕ PAUSE', '☕ BREAK')}</button>
            </div>
          )}
          {isRunning && isOnBreak && currentEmployeeId && (
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button onClick={() => endBreak(currentEmployeeId)} style={{ borderRadius: '999px', border: '2px solid var(--success)', color: 'var(--success)', background: 'transparent', padding: '12px 32px', fontSize: '13px', cursor: 'pointer', fontWeight: 800, letterSpacing: '2px' }}>{t('▶ REPRENDRE', '▶ RESUME')}</button>
            </div>
          )}
        </>
      )}
        ...
        <div style={{ ...card }}>
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <p style={{ fontSize: '11px', fontWeight: 800, color: '#a855f7', letterSpacing: '2px' }}>🎯 OBJECTIF SEMAINE</p>
              {editingGoal ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ fontSize: '12px', color: '#6b7280' }}>$</span>
                  <input type="number" value={tempGoal} onChange={e => setTempGoal(e.target.value)} autoFocus style={{ width: '70px', background: 'rgba(168,85,247,0.1)', border: '1px solid rgba(168,85,247,0.4)', borderRadius: '6px', padding: '4px 8px', color: '#e9d5ff', fontSize: '13px', fontWeight: 700, outline: 'none' }}/>
                  <button onClick={() => { if (currentEmployeeId && tempGoal) setGoalTarget(currentEmployeeId, parseFloat(tempGoal)); setEditingGoal(false) }} style={{ background: 'none', border: 'none', color: '#22c55e', cursor: 'pointer', fontSize: '16px' }}>✓</button>
                  <button onClick={() => setEditingGoal(false)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: '16px' }}>✕</button>
                </div>
              ) : (
                <button onClick={() => { setTempGoal(String(goal.targetAmount)); setEditingGoal(true) }} style={{ background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.3)', borderRadius: '6px', padding: '4px 10px', color: '#a855f7', fontSize: '11px', fontWeight: 700, cursor: 'pointer' }}>
            ...
            <div style={{ padding: '8px 12px', borderRadius: '8px', marginTop: '8px', background: goalPct >= 100 ? 'rgba(34,197,94,0.15)' : goalPct >= 50 ? 'rgba(168,85,247,0.15)' : 'rgba(168,85,247,0.08)', border: `1px solid ${goalPct >= 100 ? 'rgba(34,197,94,0.3)' : 'rgba(168,85,247,0.2)'}` }}>
              <p style={{ fontSize: '12px', fontWeight: 700, color: goalPct >= 100 ? '#22c55e' : goalPct >= 50 ? '#a855f7' : '#6b7280' }}>
                {goalPct >= 100 ? '🎉 OBJECTIF ATTEINT! Vous êtes une légende!' : goalPct >= 80 ? `🔥 Presque! Il te manque ${formatCurrency(goal.targetAmount - goal.currentAmount)}` : goalPct >= 50 ? `💪 À mi-chemin! Encore ${formatCurrency(goal.targetAmount - goal.currentAmount)}` : goalPct > 0 ? `⚡ En route! ${formatCurrency(goal.targetAmount - goal.currentAmount)} restants` : '🎯 Commence à poinçonner pour progresser!'}
              </p>
            </div>
          </div>
        </div>
