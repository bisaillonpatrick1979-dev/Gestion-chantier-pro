  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", paddingBottom: "100px" }}>
      ...
      <div style={{ padding: "16px" }}>
        ...
        {activeTab === "info" && (
          <>
            {/* Infos compagnie auto */}
            <div style={{ ...cardStyle, border: "1px solid var(--primary)33", background: isXP ? "rgba(168,85,247,0.06)" : "var(--primary)08" }}>
              ...
              <div key={item.id} style={{ background: "var(--surface)", borderRadius: "10px", padding: "14px", marginBottom: "10px", border: "1px solid var(--border)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                  ...
                  {doc.lineItems.length > 1 && (
                    <button onClick={() => removeLineItem(item.id)} style={{ background: "none", border: "none", color: "var(--danger)", cursor: "pointer", fontSize: "20px", lineHeight: "1" }}>x</button>
                  )}
                </div>
                <div style={{ marginBottom: "8px" }}>
                  <label style={{ display: "block", fontSize: "11px", color: "var(--text-muted)", marginBottom: "4px", textTransform: "uppercase", letterSpacing: "0.05em" }}>Description</label>
      ...
      {showClientPicker && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", alignItems: "flex-end" }} onClick={() => setShowClientPicker(false)}>
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", width: "100%", maxHeight: "70vh", borderRadius: "20px 20px 0 0", padding: "20px", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
            ...
            {clients.length === 0 ? (
              <p style={{ color: "var(--text-muted)", textAlign: "center" }}>Aucun client. Ajoutez-en dans Clients.</p>
            ) : (
              clients.map((client) => (
