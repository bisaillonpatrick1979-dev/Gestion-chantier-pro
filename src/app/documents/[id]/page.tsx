...
// ─── SignatureCanvas — React 19 compatible ────────────────────────────────────
function SignatureCanvas({ label, isXP, onClear, canvasRef }: {
  ...
  const scale = () => {
    const e = el(); const r = rect();
    return e && r ? { sx: e.width / r.width, sy: e.height / r.height } : { sx: 1, sy: 1 };
  };
  ...
  return (
    <div style={{ background: "var(--surface)", borderRadius: "10px", padding: "14px", border: "1px solid var(--border)", marginBottom: "14px" }}>
      ...
      <canvas
        ...
        onMouseMove={(e) => {
          if (!isDrawing) return; const c = ctx(); const r = rect(); const s = scale();
          if (!c || !r) return;
          c.strokeStyle = color; c.lineWidth = 2; c.lineCap = "round";
          c.lineTo((e.clientX - r.left) * s.sx, (e.clientY - r.top) * s.sy);
          c.stroke(); c.beginPath(); c.moveTo((e.clientX - r.left) * s.sx, (e.clientY - r.top) * s.sy);
        }}
        onTouchStart={(e) => { e.preventDefault(); setIsDrawing(true); ctx()?.beginPath(); }}
        onTouchMove={(e) => {
          e.preventDefault(); if (!isDrawing) return;
          const c = ctx(); const r = rect(); const s = scale(); const t = e.touches[0];
          if (!c || !r) return;
          c.strokeStyle = color; c.lineWidth = 2; c.lineCap = "round";
          c.lineTo((t.clientX - r.left) * s.sx, (t.clientY - r.top) * s.sy);
          c.stroke(); c.beginPath(); c.moveTo((t.clientX - r.left) * s.sx, (t.clientY - r.top) * s.sy);
        }}
...
export default function DocumentPage() {
  ...
  const docType: DocumentType = (doc?.type as DocumentType) ?? "facture";
  ...
  function handleSave() {
    setSaved(true); showToastMsg("✅ Document sauvegardé!");
    setTimeout(() => { setSaved(false); router.push("/documents"); }, 1500);
  }
  function handleSendEmail() {
    if (!doc) return;
    const meta = TYPE_META[docType];
    const subject = encodeURIComponent(`${meta.label} ${doc.number} — ${company.name}`);
    ...
    upd({ status: "envoye" }); showToastMsg("📧 Email ouvert — statut → Envoyé");
  }
  function handleSendSMS() {
    if (!doc) return;
    const meta = TYPE_META[docType];
    const msg  = encodeURIComponent(`Bonjour ${doc.client.name || ""}! Votre ${meta.label.toLowerCase()} #${doc.number} de ${formatCurrency(doc.total)} est prête. — ${company.name} ${company.phone || ""}`);
  ...
  function clearCanvas(ref: React.RefObject<HTMLCanvasElement | null>) {
    const el = ref.current; if (el) el.getContext("2d")?.clearRect(0, 0, el.width, el.height);
  }
  ...
  return (
    <div style={{ minHeight:"100vh",background:"var(--bg)",color:"var(--text)",paddingBottom:"130px" }}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:"12px",padding:"16px",borderBottom:"1px solid var(--border)" }}>
        ...
        <div style={{ flex:1 }}>
          <h1 style={{ margin:0,fontSize:"18px",fontWeight:800,color:typeMeta.color }}>{typeMeta.emoji} {doc.number}</h1>
          <div style={{ fontSize:"12px",color:"var(--text-muted)",marginTop:"2px" }}>{company.name}</div>
        </div>
        <button onClick={() => setShowStatusPicker(true)} style={{ padding:"4px 10px",borderRadius:"20px",fontSize:"11px",fontWeight:700,background:STATUS_COLORS[doc.status as DocumentStatus]+"22",color:STATUS_COLORS[doc.status as DocumentStatus],border:"1px solid "+STATUS_COLORS[doc.status as DocumentStatus]+"44",cursor:"pointer" }}>
      ...
      <div style={{ display:"flex",borderBottom:"1px solid var(--border)",overflowX:"auto",scrollbarWidth:"none" }}>
        {currentTabs.map((tab) => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ flex:1,minWidth:"60px",padding:"12px 4px",border:"none",borderBottom:"2px solid transparent",background:"none",cursor:"pointer",fontSize:"11px",color:"var(--text-muted)",whiteSpace:"nowrap",...(safeTab===tab.id?tabActive:{}) }}>
            {tab.label}
          </button>
        ))}
      </div>
          ...
          <>
            ...
            {docType==="devis" && (
              <div style={{ ...cardStyle,border:"1px solid #3b82f644",background:"#3b82f608" }}>
                ...
                <p style={{ margin:0,fontSize:"14px",color:"var(--text-muted)" }}>Ce devis est valide <strong style={{ color:"var(--text)" }}>{validDays} jours</strong> à compter du <strong style={{ color:"var(--text)" }}>{doc.date}</strong>.</p>
              ...
              <div style={{ position:"relative",zIndex:1 }}>
                ...
                <div style={{ marginBottom:"16px" }}>
                  <label style={{ display:"block",fontSize:"11px",color:"var(--text-muted)",marginBottom:"4px",textTransform:"uppercase" }}>
                    {docType==="contrat"?"Acompte à la signature ($)":docType==="devis"?"Dépôt demandé ($)":"Dépôt reçu ($)"}
                  </label>
                ...
                <div style={{ background:"var(--surface)",borderRadius:"10px",padding:"16px",border:"1px solid var(--border)" }}>
                  ...
                  {doc.taxes.filter((t: {enabled:boolean}) => t.enabled).map((tax: {id:string;name:string;rate:number}) => (
                    <div key={tax.id} style={{ display:"flex",justifyContent:"space-between",marginBottom:"8px",fontSize:"14px",color:"var(--text-muted)" }}>
                      <span>{tax.name} ({tax.rate}%)</span><span>{formatCurrency((doc.subtotal-(doc.discountAmount??0))*tax.rate/100)}</span>
                    </div>
                  ))}
                  <div style={{ borderTop:"1px solid var(--border)",paddingTop:"12px",marginTop:"8px" }}>
                    ...
                    {(doc.deposit??0)>0 && (
                      <>
                        <div style={{ display:"flex",justifyContent:"space-between",fontSize:"14px",color:"#3b82f6",marginBottom:"6px" }}><span>{docType==="contrat"?"Acompte":"Dépôt"}</span><span>−{formatCurrency(doc.deposit)}</span></div>
                        <div style={{ display:"flex",justifyContent:"space-between",fontSize:"16px",fontWeight:700,color:"var(--danger)" }}><span>Solde dû</span><span>{formatCurrency(doc.balanceDue)}</span></div>
                      </>
                    )}
                  </div>
                </div>
          ...
          <div style={{ ...cardStyle,border:"1px solid #22c55e44",background:"#22c55e08" }}>
            ...
            <div style={{ position:"relative",zIndex:1 }}>
              ...
              <div style={{ marginTop:"20px",padding:"14px",background:"var(--surface)",borderRadius:"10px",border:"1px solid var(--border)" }}>
                <div style={{ fontSize:"11px",color:"var(--text-muted)",lineHeight:"1.7" }}>En signant ce document, les deux parties reconnaissent avoir pris connaissance de l'ensemble des clauses et conditions.</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:200,overflowY:"auto" }} onClick={() => setShowPreview(false)}>
          <div style={{ background:"#fff",color:"#111",margin:"20px auto",maxWidth:"600px",borderRadius:"12px",padding:"32px" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"24px" }}>
              <div>
                {company.logoUrl && <img src={company.logoUrl} alt="Logo" style={{ width:"60px",height:"60px",objectFit:"contain",marginBottom:"8px" }}/>}
